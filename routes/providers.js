...
router.get('/', async (req, res) => {
  try {
    ...
    console.log(`[${new Date().toISOString()}] Fetched ${cleanedProviders.length} providers (approved=${approved}, limit=${limit})`);
...
router.get('/admin', authenticateToken, isAdmin, async (req, res) => {
  try {
    ...
    // Clean image paths for all providers
    const cleanedProviders = providers.map(cleanImagePaths);
    console.log(`[${new Date().toISOString()}] Fetched ${cleanedProviders.length} providers for admin`);
    res.json(cleanedProviders);
  } catch (err) {
...
router.get('/admin/pending', authenticateToken, isAdmin, async (req, res) => {
  try {
    ...
    // Clean image paths for all providers
    const cleanedProviders = providers.map(cleanImagePaths);
    console.log(`[${new Date().toISOString()}] Fetched ${cleanedProviders.length} pending providers for admin`);
    res.json(cleanedProviders);
  } catch (err) {
